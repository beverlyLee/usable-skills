...
**实测结论：微信公众号与 wenyan-cli 都不会渲染 mermaid 代码块。** 直接把 ```` ```mermaid ```` 写进 Markdown 再发布，读者只会看到一坨源码，不会看到图。

**正确做法：先本地把 mermaid 渲染成图片，再以图片形式嵌入。**

1. 安装 mermaid-cli：`npm install -g @mermaid-js/mermaid-cli`
2. 准备 puppeteer 配置（无头渲染需要）：`puppeteer.json`
   ```json
   { "args": ["--no-sandbox", "--disable-setuid-sandbox"] }
   ```
3. 渲染：把 `.mmd` 源文件转成 `.svg`（公众号用 svg 更清晰，也可用 `.png`）
   ```bash
   mmdc -i flow.mmd -o flow.svg -p puppeteer.json
   ```
4. 在 Markdown 里以图片引用渲染产物（用相对/绝对路径，wenyan 会自动传图床）：
   ```markdown
   ![分层架构图](./assets/flow.svg)
   ```

**`quadrantChart`（四象限图）的特殊约束**（写文章踩过的坑）：
- 轴标签（如 `x-axis "吞吐 vs 延迟"`）含中文时**必须加引号**，否则解析报错。
- 象限标签 `quadrant-1` ~ `quadrant-4` **只能用 ASCII**，不要写成中文「象限一」。
- 示例：
  ```mermaid
  quadrantChart
    x-axis "性能 ↑" --> "成本 ↑"
...
**判断清单：发布前确认每个图都已渲染成图片文件**，而不是留下 ```` ```mermaid ```` 代码块。
...
| Skill | 分工 | 衔接点 |
|-------|------|--------|
| `wechat-toolkit` | 搜索 → 下载 → 洗稿（含去 AI 味 + 事实兜底）→ 发布 | 洗稿产出的 Markdown 直接喂给本 skill 发布 |
| `wechat-article-pro` | 联网搜热点 + 刘润风格长文 + 后台 AI 封面 + 发草稿 | 长文写好后，图表预渲染走本 skill 的流程 |
| `wechat-publisher`（本 skill） | 轻量 Markdown → 草稿箱 | 只负责「把写好的 Markdown 发上去」 |

三者是同一套公众号内容流水线的不同段：**toolkit 偏改写、article-pro 偏原创长文、publisher 偏一键发布**。本 skill 最薄，只接「带 frontmatter 的 Markdown」。

...
本 skill 每日通过美团 **EDS Claw 后端**发放的，是 **WorkBuddy（红包助手）渠道预配置的「活动券」**——
**不是你美团账户里所有可领优惠券的总和**。
...
| 券来源 | 说明 |
|--------|------|
| WorkBuddy / 红包助手渠道活动券 | 由美团通过 EDS Claw 平台下发的活动券，每人每天一次；数量与面额**随美团活动变动**。**实测曾单日领到 6 张合计 ¥75，后调整为 5 张**，以脚本返回的 `coupon_count` / `coupons` 字段为准。 |

- **领取机制**：`issue.py` 按 `(user_token + 手机号掩码 + 北京日期)` 生成每日领取码，服务端天然防重复（同一天重复调用会被 `4010 已领取` 拦截），不会多发。
- **具体券名/面额**：每张券的名称与面额会在成功 JSON 的 `coupons` 数组中返回，可在运行日志 `daily_issue.log` 或定时任务汇报里查看（内容只存本地，不入库）。

### ❌ 本 skill 暂不能领（需美团 App 内自动化）

以下权益**不在红包助手渠道内**，当前 skill 覆盖不到，需要走美团 App 内部入口（模拟器 / 真机自动化，
相关方案仍在调研/受阻中，详见项目内 `meituan/INAPP_IMPLEMENTATION.md`）：
...
- 小美果园、免单抽奖
- 美团会员券（会员月卡 / 年卡，需付费订阅）
- 第三方数字口令券（如 `333777` 等，多为推广内容，官方不稳定，**不建议依赖**）

> 说明：上面这些"每日可领权益"都是美团 App 内的常驻功能，但本 skill 依赖的 EDS Claw 接口
> **只发 WorkBuddy 渠道那批活动券**，没有查询/领取你美团账户其他券的 C 端 API
> （详见项目内 `meituan/API_RESEARCH.md`）。想把它们也自动化，得另走 App 内点击方案。

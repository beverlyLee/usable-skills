...
洗稿不是把话换种说法，而是先确认「说的对不对」。改写前必须过一遍事实关：

- **溯源**：技术/数据/案例类内容，逐条回到原文或 `web_fetch` 出来的来源；说不出处的数字、版本号、命令、架构名，一律不写进成品。
- **不编造**：原文没有的案例、数据、引言，不要为了「丰富」而补。宁可少写，不要编。
- **标注不确定**：拿不准的结论，写成「取决于 X」，不要写成确定结论。
- **这是从 8 篇技术专栏（Harbor 系列）里踩出来的**：一篇架构文只要有一处事实错，整篇可信度归零；降重再漂亮也救不回来。

> 事实兜底和去 AI 味是两个独立动作：先保证「真」，再去「味」。顺序不能反。

### 多图型规划（长文必做）

一篇能读下去的长文，默认要配 2–3 种图型，不要纯文字堆：

- **架构/流程类**：用 mermaid `flowchart`，把分层、模块关系画出来。
- **交互/时序类**：用 mermaid `sequenceDiagram`，画「谁先说、谁后说」。
- **对比/分类类**：用 mermaid `mindmap` 或 `quadrantChart`。
- **状态机类**：用 mermaid `stateDiagram-v2`。

每篇至少 1 张图，技术长文建议 2–3 张，分散在章节之间，别堆在开头。

### mermaid 预渲染（发公众号前必做）

**公众号后台和 wenyan-cli 都不会渲染 mermaid 代码块**——直接贴 ```` ```mermaid ```` 只会显示成纯文本。两条路二选一：

- **路 A（推荐，发草稿用）**：先把 mermaid 渲染成图片再嵌入。本地用 mermaid CLI 校验并出图：
  ```bash
  mmdc -i diagram.mmd -o diagram.svg -p puppeteer.json
  # SVG 转 PNG（公众号图床更稳）：用你惯用的转图工具
  ```
  然后正文里用 `![图注](/绝对路径/diagram.png)` 引用。**渲染时必须 `mmdc` 真跑一遍**，否则中文标签容易翻车（见下）。
- **路 B（仅本地预览用）**：用 CDN mermaid 把整篇 Markdown 渲染成 HTML 看效果，但这版不能直接发公众号。

> **mermaid 验证纪律（实测坑）**：本地 `mmdc`（mermaid@10.x）对 `quadrantChart` 有硬约束——① 轴标签含中文必须加引号（`x-axis "一次性·小" --> "反复·大规模"`），否则 Lexical error；② 象限标签（`quadrant-1`..`quadrant-4`）**不接受中文/全角括号，必须 ASCII**（如 `Repeated Eval`），中文放到 title 或数据点标签里。每张图渲染 exit 0 + 出有效 SVG，才算过。
...
## 流程分层与边界契约（这张图看懂，整条链路就通了）

四个模块不是平铺的功能，而是一条**有明确接缝的流水线**。每个接缝只传递约定的数据，上层不用管下层怎么实现：

| 接缝 | 上游 → 下游 | 穿过缝的东西 | 形态 | 谁不用管谁 |
|------|-----------|-------------|------|-----------|
| 搜索 → 下载 | 搜索结果 → 下载器 | 文章 URL 列表 | JSON（`-o result.json`） | 下载器不管搜的是啥关键词 |
| 下载 → 洗稿 | 下载产物 → 改写器 | 正文 Markdown + metadata.json + 图/视频目录 | 本地目录 | 洗稿不管图从哪抓的 |
| 洗稿 → 发布 | 改写稿 → 发布器 | 带 frontmatter 的 Markdown | `title`+`cover` 必填 | 发布器不管原文长啥样 |
| 发布 → 微信 | 发布器 → 公众号 API | 草稿 / 已发布图文 | 微信 media_id | 微信不管你用的哪个主题 |

**最关键的两条契约**：
1. **洗稿↔发布的边界 = frontmatter**：`title` 和 `cover` 缺一不可，否则 `publish.js` 直接报错。洗稿产出的最后一脚，就是补好这俩字段。
2. **发布↔微信的边界 = 凭证 + IP 白名单**：`WECHAT_APP_ID/SECRET` 在环境变量，运行机器的公网 IP 必须在公众号后台白名单（`ip not in whitelist` 就卡在这）。

换主题、换配图、换封面，都只在「发布」这一层动；上游搜索/下载/洗稿一行不用改。这就是分层的好处。
